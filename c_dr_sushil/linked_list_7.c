...
Node * concatenate_LLs(Node * p, Node * q){
    Node * head = p;
    while(p->next!=NULL){
        p=p->next;
    }
...
int main(){
    ...
    insertAtLast(&head2,&last2,-100);
    insertAtLast(&head2,&last2,5);
    insertAtLast(&head2,&last2,3);
    insertAtLast(&head2,&last2, 70);
    insertAtLast(&head2,&last2,-9);

    display(head1);
    display(head2);

    // Node* p = insertionSort(head1);
    // Node* q = insertionSort(head2);
    // display(p);
    // display(q);
    Node * merged =concatenate_LLs(head1,head2);
    merged = insertionSort(merged);
    display(merged);
